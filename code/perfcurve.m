...
if ~isempty(tVals) && ~isempty(xVals)
    error('stats:perfcurve:BothTandXsupplied',...
        'You cannot supply both T and X values.');
end
...
if docv && doboot
    error('stats:perfcurve:BothCVandBootstrapRequested',...
        'You cannot use both cross-validation and bootstrap.');
end
nsub = max(ncv,nboot);
...
if (isa(xCrit,'function_handle') || isa(yCrit,'function_handle')) && docv
    error('stats:perfcurve:UserCritConfBounds',...
        'perfcurve cannot compute cross-validation confidence bounds for user-defined X or Y criterion.');
end
...
if any(isnan(X(~special)))
    error('stats:perfcurve:BadXCritValue',...
        'Unable to compute X criterion for supplied division indices.');
end
if any(isnan(Y(~special)))
    error('stats:perfcurve:BadYCritValue',...
        'Unable to compute Y criterion for supplied division indices.');
end
...
if length(posClass)>1
    error('stats:perfcurve:TooManyPositiveClasses',...
        'Only one positive class is allowed.');
end
if ~ismember(posClass,trueNames)
    error('stats:perfcurve:PositiveClassNotFound',...
...
if strcmpi(negClass,'all')
    ...
    if any(~tf)
        error('stats:perfcurve:NegativeClassNotFound',...
            'One or more negative classes not found in the input data.');
    end
    tf = ismember(posClass,cellstr(negClass));
    if tf
        error('stats:perfcurve:PositiveAndNegativeClassesOverlap',...
...
if nNeg~=length(negClassNames)
    error('stats:perfcurve:DuplicateNegativeClasses',...
...
if isnumeric(prior)
    if any(prior<=0)
        error('stats:perfcurve:NonPositivePriors',...
...
switch lower(crit)
    ...
    case 'ecost'
        ...
        wf = @(C,scale) sum(scale.*sum(C,2));
    otherwise
        error('stats:perfcurve:UnknownXYCriterion',...
            'You specified an unknown criterion for X or Y.');
end
end
...
if ~isempty(XorTrange)
    if numel(XorTrange)~=2
        error('stats:perfcurve:InvalidXorTRange',...
            'Invalid range for X or T values.');
    end
    ...
    if isempty(inrange)
        error('stats:perfcurve:XorTRangeTooRestrictive',...
            'No X or T values in the specified range.');
    end
end
end
...
if any(vals(1:end-1)>vals(2:end))
    increasing = -1;
    if any(vals(1:end-1)<vals(2:end))
        error('stats:perfcurve:NonMonotoneXCriterion',...
...
if any(div>Nrow)
    error('stats:perfcurve:BadDivisionIndex',...
        'Division index exceeds the number of distinct points on the curve.');
end
...
if iscell(scores) % Cross-validated scores and labels
    % Scores
    if ~isvector(scores)
        error('stats:perfcurve:CVScoresNotVector',...
            'The cell array of cross-validated scores must be a vector.');
    end
    scores = scores(:);
    ncv = numel(scores);
    if ncv<2
        error('stats:perfcurve:CVScoresTooShort',...
            'The cell array of cross-validated scores must have at least 2 elements.');
    end
    ...
    if any(tfisemp) || any(~tfisnum) || any(~tfisvec)
        error('stats:perfcurve:CVScoresWithBadElements',...
            'The cell array of cross-validated scores must be composed of non-empty numeric vectors.');
    end
    ...
    % Labels
    if ~iscell(labels)
        error('stats:perfcurve:CVLabelsNotCell',...
            'Cross-validated labels must be supplied as a cell array.');
    end
    if ~isvector(labels)
        error('stats:perfcurve:CVLabelsNotVector',...
            'The cell array of cross-validated labels must be a vector.');
    end
    if numel(labels)~=ncv
        error('stats:perfcurve:CVLabelsWithNonmatchingLength',...
            'The number of cross-validated label vectors must match the number of cross-validated score vectors.');
    end
    labels = labels(:);
    ltype = cellfun(@class,labels,'UniformOutput',false);
    for icv=2:ncv
        if ~strcmp(ltype{1},ltype{icv})
            error('stats:perfcurve:CVLabelsWithDifferentTypes',...
                'Vectors of cross-validated labels must have the same type.');
        end
    end
    nell = zeros(ncv,1);
    for icv=1:ncv
    ...
    if isempty(weights)
        ...
        if ~iscell(weights)
            error('stats:perfcurve:CVWeightsNotCell',...
                'Cross-validated weights must be supplied as a cell array.');
        end
        if ~isvector(weights)
            error('stats:perfcurve:CVWeightsNotVector',...
                'The cell array of cross-validated weights must be a vector.');
        end
        if numel(weights)~=ncv
            error('stats:perfcurve:CVWeightsWithNonmatchingLength',...
                'The number of cross-validated weight vectors must match the number of cross-validated score vectors.');
        end
        weights = weights(:);
    end
    for icv=1:ncv
    ...
    if any(~tfisflo) || any(~tfisvec)
        error('stats:perfcurve:CVWeightsNotNumeric',...
            'The cell array of cross-validated weights must be composed of floating-point vectors.');
    end
    fneg = @(x) any(x<0);
    fzer = @(x) all(x==0);
    fnan = @(x) all(isnan(x));
    ...
    if any(tfisneg) || any(tfiszer) || any(tfisnan)
        error('stats:perfcurve:CVWeightsNegativeOrNaN',...
            'Each vector of cross-validated weights must have non-negative, non-NaN elements with at least one positive element.');
    end
    nelw = cellfun(@numel,weights);
    
    % Check sizes
    if any(nels~=nell) || any(nelw~=nell)
        error('stats:perfcurve:CVWeightsNotMatchedToScores',...
            'The size of at least one vector of cross-validated labels does not match the size of corresponding scores.');
    end
    ...
    % Scores
    if ~isvector(scores)
        error('stats:perfcurve:ScoresNotVector',...
    ...
    if ~ischar(labels)
        if ~isvector(labels)
            error('stats:perfcurve:LabelsNotVector',...
    ...
    nl = size(labels,1);
    if ns~=nl
        error('stats:perfcurve:ScoresAndLabelsDoNotMatch',...
    ...
    if isempty(weights)
        ...
        if ~isfloat(weights)
            error('stats:perfcurve:WeightsNotRealNumbers',...
                'Weights must be floating-point numbers.');
        end
        if ~isvector(weights)
            error('stats:perfcurve:WeightsNotVector',...
                'Array of weights must be a vector.');
        end
        if any(weights<0) || all(weights==0)
            error('stats:perfcurve:WeightsNegative',...
                'Array of weights must have non-negative elements with at least one positive element.');
        end
        if numel(weights)~=ns
            error('stats:perfcurve:WeightsNotMatchedToScores',...
                'The size of scores does not match the size of weights.');
        end
        if any(isnan(weights))
            error('stats:perfcurve:NaNWeights',...
                'Array of weights has NaN elements.');
        end
        weights = weights(:);
        iszer = weights==0;
        scores(iszer) = [];
...
if ~all(sum(Wcum,1)) && notifyClassAbsent()
    warning('stats:perfcurve:SubSampleWithMissingClasses',...
        'One of the classes is not present in at least one subsample. You may get NaN confidence bounds.');
end
...
tfzeroW = ~any(W,2);
if any(tfzeroW)
    warning('stats:perfcurve:CVfoldsWithZeroWeights',...
        'Some rows in WSTD do not have any non-NaN elements with positive weights.');
end
E = zeros(length(M),1);
i = ~tfzeroW;
